...
from trainer2 import QTrainer as DoubleQTrainer
import pygame
import cv2
...
class AdvancedAgent:
    def __init__(self, algorithm='DQN'):
        ...
        # IMPROVED: Much more aggressive strategy
        self.epsilon = 0.5   # Start much lower  
        self.epsilon_min = 0.01  # Very low minimum
        self.epsilon_decay = 0.990  # Much faster decay
        self.gamma = 0.99  
    ...
    def get_action(self, state, training=True, game_state=None):
        """QUICK FIXED: Simple safe exploration - NO MORE STUCK!"""
        final_move = [0, 0, 0, 0]
        ...
        if training and random.random() < self.epsilon:
            # SIMPLE SAFE EXPLORATION thay vì complex smart strategies
            if game_state is not None:
                ...
                # Choose random from safe actions
                if safe_actions:
    ...
    def adaptive_epsilon_decay(self, win_rate):
        """IMPROVED: Very aggressive epsilon decay to force exploitation"""
        if win_rate > 0.15:  # Any decent performance -> extremely fast decay
            decay_rate = 0.95
        elif win_rate > 0.05:  # Some progress -> very fast decay
            decay_rate = 0.97
        elif win_rate > 0.01:  # Minimal progress -> fast decay
            decay_rate = 0.98
        else:  # No progress -> aggressive decay to stop random exploration
            decay_rate = 0.985
...
class TrainingManager:
    ...
    def train_single_level(self, level_name='Level1', algorithm='DQN', total_episodes=5000, eval_interval=1000, eval_games=100):
        """Train một level cụ thể với một thuật toán cụ thể"""
        
        # Mapping level names to classes
        game_classes = {
            'Level1': Level1AI,
            'Level2': Level2AI, 
            'Level3': Level3AI
        ...
        if algorithm not in ['DQN', 'Double_DQN']:
            print(f"❌ Algorithm {algorithm} không hợp lệ. Chọn: DQN hoặc Double_DQN")
            return None
            
        game_class = game_classes[level_name]
        
        print(f"\n🚀 Training {algorithm} on {level_name}")
        print(f"📊 Episodes: {total_episodes}, Eval every: {eval_interval}")
        print("=" * 50)
        
        # Tạo agent
        agent = AdvancedAgent(algorithm=algorithm)
        
        # Load model và experience nếu có
        ...
        while episode < start_episode + total_episodes:
            ...
            # IMPROVED: Adaptive epsilon decay based on EVALUATION performance
            if episode % 100 == 0:
                # Use evaluation win rate instead of training win rate
            ...
            if episode % eval_interval == 0:
                ...
                if current_win_rate > best_win_rate:
                    ...
                    print(f"📊 Current best: {best_win_rate:.2%} (Episode {best_model_episode})")
                
                # Update target network less frequently to prevent forgetting
                if episode % (eval_interval * 2) == 0:  # Every 2 evaluations instead of every eval

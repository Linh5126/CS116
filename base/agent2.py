...
class Agent:
    ...
    def get_state(self, game):
        ...
        state = [
            ...
            game.food_x < game.head.x,
            game.food_x > game.head.x,
            game.food_y < game.head.y,
            game.food_y > game.head.y
        ]
...
def train2(game=Level2AI(), num_games=1000):
    ...
    record = 0
    agent = Agent()
    if isinstance(game, Level1AI): agent.load_state("training_state_db_dqn_lv1.pkl")
    elif isinstance(game, Level2AI): agent.load_state("training_state_db_dqn_lv2.pkl")
    elif isinstance(game, Level3AI): agent.load_state("training_state_db_dqn_lv3.pkl")
    a= "Double DQN"
    frames = []
    while True:
        ...
        if done:
            ...
            #   elif isinstance(game, Level3AI): agent.save_state("training_state_db_dqn_lv3.pkl")
            ...
            if score > record:
                ...
                if isinstance(game, Level1AI): 
                    ...
                    last_best_video = f"videos1/best_gamelv3_db_dqn_{agent.n_games}.mp4"
                    save_video_from_frames(frames, last_best_video)
                    delete_old_videos(last_best_video, prefix='best_gamelv3_db_dqn_')
            if score == 10: nw = nw + 1
            frames = []
    ...
    elif isinstance(game, Level3AI): final_chart_path = f"plots/double_dqn_lv3_{num_games}.png"
